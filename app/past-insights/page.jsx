...
export default function PastInsights() {
    ...
    const fetchInsights = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/past-insights"
        );
        console.log("Response is ", response);
        setFileurl(response.data.fileurl);
        setDate(response.data.createdAt)

        const insightsArray = Array.isArray(response.data.body)
          ? response.data.body
          : [response.data.body];
        // console.log("Processed Insights Array:", insightsArray);
        setDataResponse(insightsArray);
  ...
  return (
    <div className="min-h-screen p-8 pt-20">
              ...
              >
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {insight.Skills.Technical.slice(0, 3).map((skill, i) => (
                      <span
                        key={i}
      ...
      {showModal && (
        <Modal setShowModal={setShowModal} dataResponse={selectedInsight} />
      )}
    </div>
  );
}
